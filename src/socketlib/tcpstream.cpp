...
bool TCPStream::waitForWriteEvent(int timeout)
{
    fd_set write_flags;
    struct timeval tv;
    ...
    tv.tv_usec = 0;
    FD_ZERO(&write_flags);
    FD_SET(m_sd, &write_flags);
    if (select(m_sd+1, NULL, &write_flags, NULL, &tv) > 0)
    {
        return true;
    }
